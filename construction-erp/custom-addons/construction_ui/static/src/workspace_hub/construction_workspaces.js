...
// The Construction and Facilities workspaces. Configuration only: the hub
// component, the area lookup and the registration helper live in
// majal_suite_ui, which knows nothing about either application. That is what
// lets a customer who bought only Property install the shell without this.
...
CONSTRUCTION_METRICS.quality_hub = CONSTRUCTION_METRICS.defects;
CONSTRUCTION_METRICS.site_work = CONSTRUCTION_METRICS.inspections;
CONSTRUCTION_METRICS.engineering_hub = CONSTRUCTION_METRICS.drawings;

const CONSTRUCTION_WORKSPACES = {
    projects: workspace({
...
// Each workspace records which module owns its client action, so a related
// tile pointing into another suite still resolves to the right module.
for (const config of Object.values(CONSTRUCTION_WORKSPACES)) {
    config.hubModule = "construction_ui";
}

Object.assign(AREAS, CONSTRUCTION_AREAS);
Object.assign(OPERATIONAL_METRICS, CONSTRUCTION_METRICS);
Object.assign(WORKSPACES, CONSTRUCTION_WORKSPACES);
registerWorkspaces(Object.keys(CONSTRUCTION_WORKSPACES));
