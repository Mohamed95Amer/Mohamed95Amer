...
export async function POST(request: Request) {
  ...
  const [{ data: vendor }, { data: buyerRequest }] = await Promise.all([
    admin.from("vendors").select("id, verification_status").eq("owner_user_id", auth.user.id).maybeSingle(),
    admin.from("buyer_requests").select("id, status, expires_at").eq("id", parsed.data.requestId).maybeSingle(),
  ]);
  if (!vendor || vendor.verification_status !== "approved") return NextResponse.json({ error: "approved_vendor_required" }, { status: 403 });
