...
import { adminVendorDecisionSchema } from "@/lib/validation/schemas";
...
export async function POST(request: Request) {
  ...
  const parsed = adminVendorDecisionSchema.safeParse(body);
  ...
  const { data: prev } = await admin.from("vendors").select("verification_status").eq("id", parsed.data.vendorId).single();

  const { error } = await admin
    .from("vendors")
    .update({ verification_status: nextStatus, admin_notes: parsed.data.note ?? null })
  ...
  await logAudit({
    ...
    action: `vendor.${parsed.data.decision}`,
    entity_type: "vendor",
    entity_id: parsed.data.vendorId,
    old_value: { verification_status: prev?.verification_status },
