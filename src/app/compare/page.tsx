...
export default async function ComparePage({ searchParams }: { searchParams: Promise<{ ids?: string }> }) {
  ...
  const rows = ordered.map((product) => {
    const breakdown = computePrice({ pricePerGram24kAed: rate, karat: Number(product.karat), weightGrams: Number(product.weight_grams), makingCharge: Number(product.making_charge), makingChargeDiscountPercent: Number(product.making_charge_discount_percent), makingChargeOfferEndsAt: product.making_charge_offer_ends_at, certificateFee: Number(product.certificate_fee), stoneValue: Number(product.stone_value), vendorPremium: Number(product.vendor_premium), vendorRateAdjustmentPerGram: Number(product.vendor_rate_adjustment_per_gram ?? 0), platformFeeBps: feeOffer.effectiveBps, deliveryFee: applyEventDeliveryDiscount(Number(settings?.delivery_fee_aed ?? 0), feeOffer.eventDeliveryDiscountPercent), vatRateBps: Number(product.vat_rate_bps ?? 500) });
