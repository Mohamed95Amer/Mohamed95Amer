        ...
        <tbody>
          {(data ?? []).map((v) => (
            <tr key={v.id} className="border-t border-bone-deep">
              ...
              <td className="px-4 py-2"><div className="flex flex-wrap gap-1">{promoted.has(v.id) && <span className="pill border-gold-300/50 bg-gold-50 text-gold-700">{arabic ? "إعلان · حتى " : "Ad · until "}{new Intl.DateTimeFormat("en-AE", { timeZone: "Asia/Dubai", day: "numeric", month: "short" }).format(new Date(promoted.get(v.id)!.endsAt))}</span>}<span className={`pill ${v.store_latitude != null && v.store_longitude != null ? "border-signal-ok/30 bg-signal-ok/10 text-signal-ok" : "border-signal-warn/30 bg-signal-warn/10 text-signal-warn"}`}>{v.store_latitude != null && v.store_longitude != null ? (arabic ? "الدبوس محفوظ" : "Pin saved") : (arabic ? "أضف الدبوس" : "Pin needed")}</span></div><span className="mt-1 block text-xs text-ink-muted">{[v.contact_first_name, v.phone, v.trade_license_number, v.delivery_available || v.online_payment_available].filter(Boolean).length}/4 {arabic ? "خطوات جاهزية" : "onboarding checks"}</span></td>
