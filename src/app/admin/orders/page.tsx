          ...
          {(data ?? []).map((o) => {
            ...
            return (
              <tr key={o.id} className="border-t border-bone-deep">
                ...
                <td className="px-4 py-2">{o.payment_method === "aani" ? "Aani → store" : o.payment_method === "bank_transfer" ? "Bank → store" : o.payment_method === "pay_online" ? "Online" : "Direct to store"}<span className="block text-[10px] text-ink-muted">{statusLabel(o.payment_status)}</span></td>
