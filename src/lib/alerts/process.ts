...
export async function processDuePriceAlerts(limit = 100) {
  ...
  for (const alert of alerts) {
    ...
    const breakdown = computePrice({ pricePerGram24kAed: Number(tick.price_per_gram_24k_aed), karat: Number(product.karat), weightGrams: Number(product.weight_grams), makingCharge: Number(product.making_charge), makingChargeDiscountPercent: Number(product.making_charge_discount_percent), makingChargeOfferEndsAt: product.making_charge_offer_ends_at, certificateFee: Number(product.certificate_fee), stoneValue: Number(product.stone_value), vendorPremium: Number(product.vendor_premium), vendorRateAdjustmentPerGram: Number(product.vendor_rate_adjustment_per_gram ?? 0), assayFineness: product.assay_fineness == null ? null : Number(product.assay_fineness), platformFeeBps: applyEventFeeDiscount(Number(settings?.platform_fee_bps ?? 100), promotion?.serviceFeeDiscountPercent ?? 0), deliveryFee: applyEventDeliveryDiscount(Number(settings?.delivery_fee_aed ?? 0), promotion?.deliveryDiscountPercent ?? 0), vatRateBps: Number(product.vat_rate_bps ?? 500) });
